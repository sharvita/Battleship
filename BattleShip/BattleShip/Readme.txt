...
game of battleship against the computer. The progam allows
th user to input a file to enter ship locations. After
genrating random position for the computer ships, it asks
...
Name: Grid.cpp
   ...
   contains info about the placament of ships, randomly shoots 
   torpedos for computer, allows to check for placement of ship,
...
Name:  Computer.cpp
   Randomly generates number so that the ships can be 
...
Name:  Player.cpp
   ...
   and sends this info to grid to see if ships are placable.
   Allows the user to attack the computer grid and checks if
   ship has been hit. 
...
*******************************************************
   ...
   The battleship game allows the user to input file but
...
does not fully function for user to play battleship. The program
is was tested on cse grid but does not compile.
